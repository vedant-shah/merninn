...
function Book() {
  ...
  const [drpVal, setDrpVal] = useState(
    checkin.toString().substring(3, 15) +
      " - " +
      checkout.toString().substring(3, 15)
  );
  const location = [
    {
      value: "Goa",
  ...
  async function postData() {
    await fetch(url, {
      ...
      showAlert(
        "Congratulations, your Holiday has been successfully booked!",
        "success"
      );
    });
  }
    ...
    <>
      <form className="d-flex flex-column">
        ...
        <TextField
          ...
          value={phone}
          error={isPhoneError}
          helperText={isPhoneError ? phoneErrorMessage : ""}
          onChange={(event) => {
        ...
        <TextField
          ...
          value={room}
          error={isRoomError}
          helperText={isRoomError ? roomErrorMessage : ""}
          onChange={(event) => {
  ...
  return (
    <>
      <div className="card mx-auto d-flex mt-5 px-2 py-3 col-md-7 col-sm-12 col-xs-12">
        ...
        <hr />
        <div className="h5 mx-auto" style={{ fontWeight: "bold" }}>
          Booking Details:
        </div>
        {getCardContent(c)}
        ...
        <p className="mx-auto" style={{ color: "red", fontSize: "0.8rem" }}>
          {isError ? "One or more fields are invalid, kindly re-check" : ""}
        </p>
      </div>
    </>
  );
